...
export async function notif(){
    ...
    await process.stdout.write(
        chalk.bold.bgGreen(`\r [${time}] Inbox `) + chalk.bold.bgYellow(` ${mf.type} `) + ' '.repeat(100)
        + (mf.is.group ? chalk.bold.bgCyan(`\n Group: `) + ` ${mf.group.id} ` + chalk.cyan(`${mf.group.name}`) : '')
        + chalk.bold.bgCyan(`\n contact: `) + ` ${mf.phoneNumber} ` + chalk.cyan(`${mf.name}`)
        + (mf.is.noBody ? '\n\n' : chalk.bold.bgYellow(`\n Inbox: `) + (mf.is.cmd ? chalk.yellow(` "${mf.body}"\n\n`) : ` "${mf.body}"\n\n`))
    )
}
    ...
    for (let file of files) {
        ...
        if (!(filename in db.data.plugins)) {
            db.data.plugins[filename] = {
                ...
                tag: handler.tag ? handler.tag : 'Other',
